            ...
            if(*(key+size) == '=' && thread_info->state == 1)
            {
                *(key+size) = '\0';
                ...
                while(redis_state == 0)
                {
                    for(int j=0;j<4;j++)
                    {
                        if(redis_info[(j+thread_info->redis_shift)%4+thread_info->index_num*4].working == -1)
                        {
                            redis_bev = redis_bev_list[(j+thread_info->redis_shift)%4+thread_info->index_num*4];
                            bufferevent_disable(redis_bev, EV_WRITE | EV_READ);
                            redis_state=1;
                            redis_info[(j+thread_info->redis_shift)%4+thread_info->index_num*4].working=0;
                            thread_info->redis_index = (j+thread_info->redis_shift)%4+thread_info->index_num*4;
                            redis_index = (j+thread_info->redis_shift)%4+thread_info->index_num*4;
                            redis_info[(j+thread_info->redis_shift)%4+thread_info->index_num*4].sock_bev=thread_info->sock_bev;
                            //printf("redis %d is picked for %d\n", thread_info->redis_index, thread_info->sockfd);
                            thread_info->redis_shift+=1;
                            break;
                        }
                    }
                }
...
void *redis_thread_task(void *info)
{
    struct _rh *redis = info;
    struct event_base* base;

    base = event_base_new();
    redis->base = base;
    event_base_loop(base, 4);
}
...
void *thread_task(void *info)
{
    ...
    {   
        ...
        /* when the signal is come thread wakes and get socket fd of client */
        fd = thread_info->sockfd;
        //printf("thread run %d\n", thread_info->index_num);
        //pthread_mutex_unlock(&thread_info->lock);
        if(fd > 0)
        {
            /* Make bufferevent to do task */
            ...
            bufferevent_disable(bev, EV_WRITE);
            bufferevent_setwatermark(bev, EV_WRITE, 1, 100); 
            event_base_dispatch(base);
            //printf("Thread %d end\n", thread_info->index_num);
            //pthread_mutex_unlock(&thread_info->lock);
        }
...
/* dequeue task and allocate to task thread */
int dequeue_task(pthread_mutex_t lock, int shift)
{
    ...
    pthread_mutex_unlock(&lock);
    return (end_queue-front_queue+MAX_queue)%MAX_queue;
}

/* enqueue task and return queue length */
int enqueue_task(int fd, pthread_mutex_t lock, int shift)
{
    pthread_mutex_lock(&lock);
    //printf("queue %d\n", front_queue-end_queue);
    ...
    pthread_mutex_unlock(&lock);
    //printf("len %d", end_queue-front_queue);
    return (end_queue-front_queue+MAX_queue)%MAX_queue;
}
...
int main(int argc, char *argv[])
{
    ...
    redis_thread = (pthread_t *)malloc(sizeof(pthread_t) * 10);
    redis_thread_info = (struct _rh *)malloc(sizeof(struct _rh) * 10);

    for(int i=0;i<10;i++)
    {
        pthread_cond_init(&redis_thread_info[i].cond, NULL);
        pthread_mutex_init(&redis_thread_info[i].lock, NULL);
        redis_thread_info[i].num=i;
        if(pthread_create(&redis_thread[i], NULL, redis_thread_task, (void *)&redis_thread_info[i])<0)
        {
            printf("thread create failed\n");
            return -1;
        }
    ...
    for(int i=0;i<40;i++)
    {
        ...
        redis_bev_list[i] = bufferevent_socket_new(redis_thread_info[num].base, client_socket, BEV_OPT_CLOSE_ON_FREE | O_NONBLOCK);
        bufferevent_setcb(redis_bev_list[i], redis_read_cb, NULL, NULL, (void *)&redis_info[i]); 
        redis_info[i].sockfd = -1;
        redis_info[i].redisfd = client_socket;
        redis_info[i].result = -1;
    ...
    {
        //printf("111\n");
        ready=select(server_socket+1, &ready_set, NULL, NULL, NULL);
